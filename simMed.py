...
def sim.cgnf(path="", dataset_name="", cat_list=[0, 1], intv_med=None, moderator=None, quant_mod=4, model_name='models', seed=None, n_mce_samples=10000, inv_datafile_name='potential_outcome'):
    ...
    with torch.no_grad():
        ...
        if confounder:
            n_confounder = len(confounder)
            # Bootstrapping to preserve correlations between confounders, and plug these bootstrapped confounder values into z_do_n
            boot_df = df.sample(n=n_mce_samples, replace=True)

            loc_confounder = [variable_list.index(c) for c in confounder]
            ...
            indices = [loc_treatment] + loc_confounder
            ...
            for k in range(n_confounder):
                # Access the bootstrapped values for each confounder using indices from loc_confounder
                all_c_values[k] = boot_df.iloc[:, loc_confounder[k]].values
                # Convert these values into a PyTorch tensor
                all_c_list[k] = torch.tensor(all_c_values[k], dtype=torch.float).to(device)
                all_c_tensor[k] = all_c_list[k].unsqueeze(-1).unsqueeze(-1)
                # Expand the tensor to match z_do_n's shape
                all_c_tensor[k] = all_c_tensor[k].expand(-1, all_a.shape[0], -1).clone().to(device)
                # Insert the bootstrapped confounder values into z_do_n
                z_do_n[:, :, [loc_confounder[k]]] = all_c_tensor[k]
    ...
    with torch.no_grad():
        if confounder:
            cur_x_do_inv = model.invert(z_do_n, do_idx=[loc_treatment] + loc_confounder,
        ...
        if mediator:
            ...
            for i in range(n_mediators):
                for val in ['0', '1']:  # Control and treatment
                    ...
                    for j in range(i + 1):  # Include all mediators up to i
                        if mediator_values[j] is not None:  # If a specific value is provided
                            if mediator_values[j] == "intv":  # Checking for interventional effect flag
                                ...
                                if confounder:
                                    for k in range(n_confounder):
                                        z_do_n_js[j][:, :, [loc_confounder[k]]] = all_c_tensor[k]

                                z_do_n_js[j] = z_do_n_js[j].transpose_(1, 0).reshape(-1, dim).to(device)

                                if confounder:
                                    cur_x_do_inv_js[j] = model.invert(z_do_n_js[j], do_idx=[loc_treatment] + loc_confounder,
                                                                do_val=torch.index_select(z_do_n_js[j], 1, indices_tensor))
                                else:
                    ...
                    if confounder:
                        for k in range(n_confounder):
                            do_values.append(all_c_list[k])

                    do_val = torch.stack(do_values, -1)

                    if confounder:
                        cur_x_med = model.invert(z_do_n_med, do_idx=[loc_treatment] + loc_mediator[:i + 1] + loc_confounder,
                                                 do_val=do_val)
                    else:
            ...
            if moderator:
                ...
                if len(unique_moderator_values) > 10:
                    quartile_labels, quartile_intervals = pd.qcut(inv_output[moderator], q=quant_mod,
                                                                  retbins=True)
